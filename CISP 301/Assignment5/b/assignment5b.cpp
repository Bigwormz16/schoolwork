...
int main(void)
{
	    ...
	    do 
		{
			...
			printf(FORMAT,employee[empcount].fullName,employee[empcount].payrate,
		...
		{
			reportData(reportFile,employee[index[i]].fullName,employee[index[i]].payrate,
					   employee[index[i]].hours,employee[index[i]].gross,employee[index[i]].tax,
...
void reportFinish(FILE *reportFile,float totpay,float tothrs,float totgro,float tottax,float totnet,int empcount,
				  char fullName[],float payrate,float hours,float gross,float tax,float net,int index[]) //3.9
{                                                                                                               ) //3.9
